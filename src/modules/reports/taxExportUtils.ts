...
export function exportTaxReportToExcel(taxReportData: TaxReportData) {
  const { year, totalRentalIncome, totalDeductions, netTaxableIncome, propertyReports } = taxReportData;
  ...
  const detailsData = [
    ...
    ['Año:', year.toString()],
    [''],
    ['Vivienda', 'Ingresos por Alquiler', 'Gastos Deducibles', 'Rendimiento Neto'],
  ];
...
export function exportTaxReportToPDF(taxReportData: TaxReportData) {
  ...
  autoTable(doc, {
    startY: finalY + 10,
    head: [['Vivienda', 'Ingresos por Alquiler', 'Gastos Deducibles', 'Rendimiento Neto']],
    body: tableData,
    theme: 'grid',
    styles: { fontSize: 9 },
    headStyles: { fillColor: [41, 128, 185] },
